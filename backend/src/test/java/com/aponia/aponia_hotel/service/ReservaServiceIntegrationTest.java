...
class ReservaServiceIntegrationTest {
    ...
    void setUp() {
        ...
        cliente = new Usuario(UUID.randomUUID().toString(), "cliente@test.com", "123456", UserRole.CLIENTE, null, null);
        usuarioRepository.save(cliente);

        tipo = new HabitacionTipo(UUID.randomUUID().toString(), "Suite Premium", "Vista al mar", 2,
                new BigDecimal("500000"), true, null, null);
        tipoRepository.save(tipo);

        habitacion = new Habitacion(UUID.randomUUID().toString(), tipo, (int) (Math.random() * 1000), true);
        habitacionRepository.save(habitacion);
    }

    @Test
    void crearReservaCliente_creaCorrectamente() {
        Reserva reserva = reservaService.crearReservaCliente(
                cliente.getId(),
                tipo.getId(),
        ...
        assertThat(reserva.getCliente().getEmail()).isEqualTo("cliente@test.com");
        assertThat(reserva.getEstado()).isEqualTo(EstadoReserva.CONFIRMADA);
        assertThat(reserva.getEstancias()).isNotEmpty();
        assertThat(reserva.getEstancias().get(0).getTipoHabitacion().getNombre()).isEqualTo("Suite Premium");
    }
